      ...
      <div className="flex justify-between items-center mb-2 mt-2">
        <div className="flex">
          {Object.keys(projectLinks).map((key) => (
            <Link className="border border-white bg-yellow-400 text-black px-3 py-1 mr-2 inline-block" key={key + projectLinks[key]} to={projectLinks[key]}>
              {key}
            </Link>
          ))}
        </div>
        {/* <Link className="border border-white bg-yellow-400 text-black p-2 mr-2" to={github_repo}>Github</Link> */}
        <p className="text-lg font-bold">{tech_stack.join(", ")}</p>
      </div>
    </div>
  );
};
